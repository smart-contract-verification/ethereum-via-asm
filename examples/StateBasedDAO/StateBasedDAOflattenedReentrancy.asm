asm StateBasedDAOflattenedReentrancy
			...
			let ($scl = sender($cl)) in
				if executing_function($cl) = attack then
					switch instruction_pointer($cl)
						case 0 : 
							let ($s = attacker, $r = random_user, $f = random_function) in
								par
									if balance($s) >= 0 and 0>=0 and $s != $r then 
										par
											balance($s) := balance($s) - 0 
											balance($r) := balance($r) + 0
										endpar
									else 
										exception := true
									endif
									...
									if is_contract($r) then
										par
											sender(current_layer + 1) := $s // set the transition attribute to the sender user
											amount(current_layer + 1) := 0 // set the transaction attribute to the amount of coin to transfer
											current_layer := current_layer + 1
											executing_contract(current_layer + 1) := $r
											executing_function(current_layer + 1) := $f
											instruction_pointer(current_layer + 1) := 0
									...
									if is_contract($r) and 0 > 0 and not payable($f) then
										exception := true
									endif
								endpar
							endlet
	...
	rule r_Fallback_attacker = 
		let ($cl = current_layer) in
			if executing_function($cl) != attack then
				switch instruction_pointer($cl)
					case 0 : 
						let ($s = attacker, $r = sender($cl), $f = random_function) in
							par
								if balance($s) >= 0 and 0>=0 and $s != $r then 
									par
										balance($s) := balance($s) - 0 
							...
							par 
								...
								if is_contract($r) then
									par
										...
										amount(current_layer + 1) := customer_balance($r)
										current_layer := current_layer + 1
										executing_contract(current_layer + 1) := $r
										executing_function(current_layer + 1) := $f
										instruction_pointer(current_layer + 1) := 0
