...
definitions:
	...
	rule r_Bid = 
		if executing_function(current_layer) = bid then 
			switch instruction_pointer(current_layer)
				case 0 : 
					r_Require[amount(current_layer) = currentBid(global_state_layer) + 1]
				case 1 :
				...
				case 5 : 
					par
						currentBid(global_state_layer) := currentBid(global_state_layer) + 1
						instruction_pointer(current_layer) := instruction_pointer(current_layer) + 1
					endpar
				case 6 : 
					r_Ret[]
    ...
    rule r_Withdraw = 
        ...
        switch instruction_pointer(current_layer)
            case 0 : 
                r_Require[balance(auction, global_state_layer) >= 10]
            case 1 : 
                par
                    r_Transaction[auction, owner(global_state_layer), balance(auction, global_state_layer), fallback]
                    currentFrontrunner(global_state_layer) := owner(global_state_layer)
                    currentBid(global_state_layer) := 0
                endpar
            case 2 : 
                r_Ret[]
        endswitch
...
default init s0:
	...
	function balance($c in User, $n in StackLayer) = if $n = 0 then if $c = auction then 0 else 10 endif endif
